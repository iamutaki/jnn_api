...
export const subDistrictController = {
  ...
  getOne: async (c: Context<Env>) => {
    ...
    if (!item) {
      return response.error(c, 'Sub-distrik tidak ditemukan', 404, 'SUB_DISTRICT_NOT_FOUND')
    }
  ...
  create: async (c: Context<Env>) => {
    ...
    if (!districtExists) {
      return response.error(c, 'Distrik tidak ditemukan', 400, 'SUB_DISTRICT_DISTRICT_NOT_FOUND')
    }

    const nameExists = await subDistrictService._nameExists(c.env.DB, result.body.name as string)
    if (nameExists) {
      return response.error(c, 'Nama sudah digunakan', 409, 'SUB_DISTRICT_NAME_EXISTS')
    }
  ...
  update: async (c: Context<Env>) => {
    ...
    if (id === subDistrictService.GLOBAL_SUB_DISTRICT_ID) {
      return response.error(c, 'Tidak bisa mengubah sub-distrik Global', 403, 'SUB_DISTRICT_PROTECTED')
    }
  ...
  remove: async (c: Context<Env>) => {
    ...
    if (id === subDistrictService.GLOBAL_SUB_DISTRICT_ID) {
      return response.error(c, 'Tidak bisa menghapus sub-distrik Global', 403, 'SUB_DISTRICT_PROTECTED')
    }
