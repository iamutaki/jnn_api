...
export const subDistrictVoucherController = {
  ...
  sync: async (c: Context<Env>) => {
    ...
    if (!Array.isArray(voucherIds)) {
      return response.error(c, 'voucherIds harus berupa daftar', 400, 'SDV_VALIDATION_ERROR')
    }

    const sdExists = await subDistrictVoucherService._subDistrictExists(c.env.DB, subDistrictId)
    if (!sdExists) {
      return response.error(c, 'Sub-distrik tidak ditemukan', 400, 'SDV_SUB_DISTRICT_NOT_FOUND')
    }

    const seen = new Set<string>()
    for (const vId of voucherIds) {
      if (typeof vId !== 'string') {
        return response.error(c, 'Setiap voucherId harus berupa teks', 400, 'SDV_VALIDATION_ERROR')
      }

      if (seen.has(vId)) {
        return response.error(c, 'Voucher sudah ada', 400, 'SDV_VALIDATION_ERROR')
      }
      seen.add(vId)

      const vExists = await subDistrictVoucherService._voucherExists(c.env.DB, vId)
      if (!vExists) {
        return response.error(c, 'Voucher tidak ditemukan', 400, 'SDV_VOUCHER_NOT_FOUND')
      }
    }
