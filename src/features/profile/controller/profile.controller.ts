...
export const profileController = {
  get: async (c: Context<Env>) => {
    ...
    if (!profile) {
      return response.error(c, 'Profil tidak ditemukan', 404, 'PROFILE_NOT_FOUND')
    }
  ...
  getReseller: async (c: Context<Env>) => {
    ...
    if (!reseller) {
      return response.error(c, 'Profil reseller tidak ditemukan', 404, 'RESELLER_NOT_FOUND')
    }
  ...
  changePassword: async (c: Context<Env>) => {
    ...
    if (result.body.newPassword !== result.body.confirmNewPassword) {
      return response.error(c, 'Password baru dan konfirmasi tidak cocok', 400, 'PROFILE_VALIDATION_ERROR')
    }
    ...
    if (!changed) {
      return response.error(c, 'Password lama salah', 401, 'PROFILE_WRONG_PASSWORD')
    }
